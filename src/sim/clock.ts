...
export interface ClockParams {
  ...
  speed: number;
  /** Score multiplier on every point awarded. */
  points: number;
  /** Boredom-fill multiplier (SLUMP: everyone wants it over with). */
  boredomRate: number;
  /** Bruno is off the platform — no pacing, no swipe, no throws — until this
   *  tick (LUNCH: out to lunch; NIGHT: asleep all round). */
  brunoAwayUntil: number;
  /** Bruno paces at half rate until this tick (MORNING: not a morning person). */
...
const PARAMS: Record<ClockMode, ClockParams> = {
  night: { label: "NIGHT", speed: 1.0, points: 2.0, boredomRate: 1.0, brunoAwayUntil: AWAY_ALL_ROUND, brunoSlowUntil: 0 },
  morning: { label: "MORNING", speed: 1.0, points: 1.0, boredomRate: 1.0, brunoAwayUntil: 0, brunoSlowUntil: 30 },
  standard: { label: "STANDARD", speed: 1.0, points: 1.0, boredomRate: 1.0, brunoAwayUntil: 0, brunoSlowUntil: 0 },
  lunch: { label: "LUNCH", speed: 1.0, points: 1.0, boredomRate: 1.0, brunoAwayUntil: 60, brunoSlowUntil: 0 },
  slump: { label: "SLUMP", speed: 0.9, points: 1.0, boredomRate: 1.6, brunoAwayUntil: 0, brunoSlowUntil: 0 },
  overtime: { label: "OVERTIME", speed: 1.25, points: 2.0, boredomRate: 1.0, brunoAwayUntil: 0, brunoSlowUntil: 0 },
};
